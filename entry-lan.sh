...
# entry-lan.sh — Modified entry script that skips SteamCMD download.
# Game files come from the shared volume. The cfg dir is a per-instance
# tmpfs mount (set in docker-compose), so writes don't affect other instances.
...
# The cfg dir is a tmpfs — populate it from the base volume backup
if [[ -d "${STEAMAPPDIR}/game/csgo/cfg-base" ]]; then
    cp -a "${STEAMAPPDIR}/game/csgo/cfg-base/." "${STEAMAPPDIR}/game/csgo/cfg/"
fi

# Install server.cfg from template
...
if [[ ! -z $CS2_HOST_WORKSHOP_COLLECTION ]] || [[ ! -z $CS2_HOST_WORKSHOP_MAP ]]; then
    CS2_MP_MATCH_END_CHANGELEVEL="+mp_match_end_changelevel true"
    CS2_STARTMAP="\<empty\>"
    CS2_MAPGROUP_ARGS=
else
