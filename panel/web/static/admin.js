...
function renderBracketLayout(container, matches, renderFn) {
    ...
    for (var r = 1; r <= maxRound; r++) {
        ...
        // Round column
        html += '<div style="display:flex;flex-direction:column;min-width:240px">';
        ...
        if (r < maxRound) {
            html += '<div class="bracket-connectors" style="display:flex;flex-direction:column;justify-content:space-around;flex:1;width:24px;min-width:24px">';
...
// Shows: (h1_team1:h1_team2 ; h2_team1:h2_team2) with blue=CT yellow=T
function formatHalfScores(game) {
    if (!game.h1ct && !game.h1t && !game.h2ct && !game.h2t) return '';
    // team1 starts CT in first half, T in second half (or vice versa)
    var t1ct = game.t1ct;
    // First half: if team1 starts CT, team1 gets h1_ct rounds, team2 gets h1_t
...
function renderAdminBracket(matches) {
    renderBracketLayout(document.querySelector('.bracket-container'), matches, renderBracketMatch);
}
...
function connectAdminBracketWS() {
    ...
    ws.onclose = function() {
        setTimeout(connectAdminBracketWS, 5000);
    };
}
